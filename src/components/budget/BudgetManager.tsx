...
export function BudgetManager() {
  ...
  const totalIncome = topLevelIncome.reduce((sum, c) => sum + (parseFloat(budgetInputs[c.id] || '0') || 0), 0);
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div className="flex items-center gap-3">
              <div className={cn(
                ...
                toBeAssigned >= 0 ? "bg-emerald-500/20" : "bg-destructive/20"
              )}>
                <Wallet className={cn(
                  "h-6 w-6",
                  toBeAssigned >= 0 ? "text-emerald-500" : "text-destructive"
                )} />
              </div>
              <div>
                <p className={cn(
                  "text-2xl font-bold",
                  toBeAssigned > 0 ? "text-emerald-500" : toBeAssigned < 0 ? "text-destructive" : "text-foreground"
                )}>
                  ${Math.abs(toBeAssigned).toFixed(2)}
                </p>
                <p className="text-sm text-muted-foreground">
                  {toBeAssigned > 0 ? 'To Assign' : toBeAssigned < 0 ? 'Over Assigned' : 'Fully Assigned'}
                </p>
              </div>
            </div>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <p className="text-muted-foreground text-center py-8">
              No expense categories found. Create categories first to set budgets.
            </p>
          ) : (
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      ...
      {/* Expense Progress Details */}
      {budgetsWithSpending && budgetsWithSpending.filter(b => 
        topLevelExpense.some(c => c.id === b.category_id)
      ).length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">Expense Progress</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {budgetsWithSpending
                .filter(b => topLevelExpense.some(c => c.id === b.category_id))
                .map(budget => {
